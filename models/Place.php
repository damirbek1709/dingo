...
class Place extends \yii\db\ActiveRecord
{
    ...
    {
        $image = Yii::$app->db->createCommand("SELECT * FROM image WHERE itemId='$this->id' AND modelName='Place'")->queryScalar();
        if ($image) {
            return true;
        }
        return false;
    ...
    public function imagesPreviewConfig()
    {
        ...
        foreach ($this->getImages() as $image) {
            $configs[] = [
                'caption' => $image->name,
                'url' => ['/place/remove-image', 'id' => $this->id, 'image_id' => $image->id],
                'key' => $image->id,
            ];
        }
        return $configs;
